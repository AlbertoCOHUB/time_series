...
leftPanel.add(ui.Label('Confirm the task in the Tasks tab →', {
...
exportButton.onClick(function(){
  if (!currentResults) { statsLabel.setValue('⚠ Run RUN first.'); return; }
  var fc = currentResults.col.map(function(img){
    ...
    return ee.Feature(null,{
      ...
      'valor': mean.get('index')
    });
  });
  Export.table.toDrive({
    collection: fc,
    description: 'serie_' + currentResults.indexName,
    fileFormat: 'CSV',
    selectors: ['fecha','valor']
  });
  statsLabel.setValue('📤 Task created → go to Tasks and press Run');
});
...
analyzeButton.onClick(function(){
  ...
  if (analysisType === 'Linear Trend') {
    ...
    switchTab('chart');
    interpretText.setValue(
      'Positive slope → index grows over time (recovery).\n' +
      'Negative slope → index decreases (degradation).\n' +
      'R² close to 1 = linear trend explains most of the change.\n' +
      'R² close to 0 = seasonal variability dominates over trend.'
    );
    return;
  }
  ...
  if (analysisType === 'Value Distribution') {
    ...
    switchTab('chart');
    interpretText.setValue(
      'Narrow high peak → homogeneous land cover.\n' +
      'Wide distribution → mixed land cover within the geometry.\n' +
      'For NDVI: >0.6 forest, 0.3–0.6 grassland, <0.2 bare soil or stress.'
    );
    return;
  }
  ...
  if (analysisType === 'Smoothed Curve') {
    ...
    switchTab('chart');
    interpretText.setValue(
      'Cubic spline smoothing — removes image-to-image noise.\n' +
      'Use it to compare seasonal amplitude across years:\n' +
      '→ Declining annual peaks = possible progressive degradation.\n' +
      '→ Rising annual troughs = improving baseline cover.'
    );
    return;
  }
...
function computeAndShowACF(vals, name) {
  ...
  if (Math.abs(peakVal) > sig) {
    ...
    lines.push('    Landsat cadence: cycle ≈ ' + mthL + ' months.');
    lines.push('    Sentinel cadence: cycle ≈ ' + mthS + ' months.');
  ...
  interpretText.setValue(lines.join('\n'));
